...
$.extend(imageOutputBinding, {
  ...
  renderValue: function(el, data) {
    ...
    var opts = {
      ...
      hoverNullOnMouseout: OR(strToBool($el.data('hover-null-on-mouseout')), false),
    ...
    if (opts.hoverId) {
      var hoverHandler = imageutils.createHoverHandler(opts.hoverId,
        ...
        opts.hoverNullOnMouseout, opts.coordmap);
...
imageutils.initCoordmap = function($el, coordmap) {
  ...
  coordmap.mouseCoordinateSender = function(inputId, clip, nullOnMouseout) {
    if (clip === undefined) clip = true;
    if (nullOnMouseout === undefined) nullOnMouseout = false;
...
imageutils.createHoverHandler = function(inputId, delay, delayType, clip,
  nullOnMouseout, coordmap)
{
  var sendHoverInfo = coordmap.mouseCoordinateSender(inputId, clip, nullOnMouseout);
  ...
  // What to do when mouse exits the image or plotting area
  var mouseout;
  if (nullOnMouseout)
